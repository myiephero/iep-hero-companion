...
// ===== MOOD RECORDS API ENDPOINTS =====

// POST /api/mood-records - Save new mood record
app.post('/api/mood-records', isAuthenticated, async (req: any, res) => {
  try {
    const userId = await getUserId(req);
    console.log('✅ PRODUCTION: Creating mood record for user:', userId);
    
    // Validate request body
    const moodSchema = z.object({
      student_id: z.string().min(1, 'Student ID is required'),
      mood: z.string().min(1, 'Mood is required'),
      mood_label: z.string().min(1, 'Mood label is required'),
      notes: z.string().optional(),
      behavior_entry: z.string().optional()
    });

    const validatedData = moodSchema.parse(req.body);
    
    // Verify student belongs to this user (security check)
    const [student] = await db
      .select()
      .from(schema.students)
      .where(and(
        eq(schema.students.id, validatedData.student_id),
        or(
          eq(schema.students.parent_id, userId),
          eq(schema.students.user_id, userId)
        )
    ...
    if (!student) {
      console.log('❌ Student not found or not authorized for user:', userId);
      return res.status(404).json({ error: 'Student not found or not authorized' });
    }

    // Create mood record
    const [newMoodRecord] = await db
      .insert(schema.mood_records)
      .values({
        user_id: userId,
        ...
        behavior_entry: validatedData.behavior_entry || null,
        created_at: new Date(),
        updated_at: new Date()
      })
      .returning();

    console.log('✅ PRODUCTION: Created mood record for student:', student.full_name);
    res.status(201).json({
      success: true,
      mood_record: newMoodRecord,
      message: `Mood record saved for ${student.full_name}`
    });
...
// GET /api/mood-records - Get mood records for user's students
app.get('/api/mood-records', isAuthenticated, async (req: any, res) => {
  try {
    const userId = await getUserId(req);
    const studentId = req.query.student_id as string;
    console.log('✅ PRODUCTION: Getting mood records for user:', userId, 'student:', studentId);

    let moodRecords;

    if (studentId) {
      // Get records for specific student (verify ownership first)
      const [student] = await db
        .select()
        .from(schema.students)
        .where(and(
      ...
      moodRecords = await db
        .select({
          id: schema.mood_records.id,
          student_id: schema.mood_records.student_id,
          mood: schema.mood_records.mood,
          mood_label: schema.mood_records.mood_label,
          notes: schema.mood_records.notes,
          behavior_entry: schema.mood_records.behavior_entry,
          created_at: schema.mood_records.created_at,
          updated_at: schema.mood_records.updated_at,
          student_name: schema.students.full_name
        })
        .from(schema.mood_records)
        .leftJoin(schema.students, eq(schema.mood_records.student_id, schema.students.id))
        .where(and(
          eq(schema.mood_records.user_id, userId),
          eq(schema.mood_records.student_id, studentId)
        ))
        .orderBy(desc(schema.mood_records.created_at));
    } else {
      // Get all mood records for user's students
      moodRecords = await db
        .select({
          id: schema.mood_records.id,
          student_id: schema.mood_records.student_id,
        ...
        .from(schema.mood_records)
        .leftJoin(schema.students, eq(schema.mood_records.student_id, schema.students.id))
        .where(eq(schema.mood_records.user_id, userId))
        .orderBy(desc(schema.mood_records.created_at));
    }

    console.log(`✅ PRODUCTION: Found ${moodRecords.length} mood records for user`);
    res.json({ mood_records: moodRecords });
    ...
    res.status(500).json({ error: 'Failed to fetch mood records' });
  }
});
